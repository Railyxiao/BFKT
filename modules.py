...
# funcsgru 是一个自定义的多层感知机（MLP）类，用于计算 GRU 中的各个门（r_t、z_t）和新状态（n_t）
...
# 改良版CE模块
class improved_ce(nn.Module):

    def __init__(self):
        super(improved_ce, self).__init__()
...
class AKT(nn.Module):
    def __init__(self, n_question, n_pid, d_model, n_blocks,
        ...
        self.g_emded = nn.Embedding(1001, 256)
        if self.separate_qa:
...
class DKVMN_C(nn.Module):
    ...
    def forward(self, q_data, qa_data, target, student_id=None):
        ...
        target = target.view(batch_size*seqlen, -1)
        ...
        return loss, read_content_embed