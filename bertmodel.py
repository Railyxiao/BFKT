...
class SeparableConv1D(nn.Module):
    def __init__(self, input_filters, output_filters, kernel_size):
        ...
        self.depthwise = nn.Conv1d(input_filters, input_filters, kernel_size=kernel_size, groups=input_filters,
...
class MonotonicConvolutionalMultiheadAttention(nn.Module):
    ...
    def dist_func(self, attention_scores, mask):
        ...
        device = 'cpu' if device < 0 else 'cuda:%i' % device
        position_effect = position_effect.to(torch.device(device))
...
class BERT(nn.Module):
    ...
    def __init__(self, hidden_size, num_head, num_encoder, max_seq_len, dropout_p=.1, use_leakyrelu=True):
        super().__init__()
    ...
    def forward(self, emb):
        emb = emb.unsqueeze(1)
        mask = torch.ones(emb.shape[0], emb.shape[1])
        z, _ = self.encoder(emb, mask)
