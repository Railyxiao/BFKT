...
parser.add_argument('--eval_every', type=int, default=0,
                    help='number of steps to evaluate, only evaluate after each epoch if 0')
...
# 创建一个 preprocess 函数，用于加载和预处理数据集。首先，加载与题目、概念相关的信息，如问题数、概念数以及每个问题涉及的最大概念数
def preprocess():
    datasets = {}
    ...
    # 设置一些参数，如最大概念数、概念数、问题数、问题的二进制表示维度
    max_concept_of_problem = 4
    concept_number = 256
    problem_number = 258896
    ...
    # 对于每个数据集划分（训练、验证、测试），检查是否已经存在预处理后的数据集文件，如果存在，则加载它；否则，创建数据集对象并保存到文件
    ...
    # 创建数据加载器，包括训练、验证和测试数据加载器，用于在训练时分批次加载数据
    loaders = {}
    for split in ['train', 'valid', 'test']:
...
if __name__ == '__main__':
    # 使用 preprocess 函数加载并预处理数据集，得到数据加载器 loaders
    try:
        loaders = preprocess()
        # 根据 args.model 的值，使用 getattr 获取指定的模型类（models 模块中定义的）
        # 如果 args.checkpoint_path 不为 'none'，则加载预训练模型否则，创建新的模型对象
        ...
        # 调用 train.train 函数，传入模型、数据加载器和参数，进行训练
